...
class MainActivity : AppCompatActivity(),LifecycleOwner, LifecycleObserver {
    ...
    private fun incrementSharedPrefCount() {
        val pref = applicationContext.getSharedPreferences("ApplicationOpenCounter", 0) // 0 - for private mode
        ...
        if (previousCounter != 0) {
            ...
            Toast.makeText(applicationContext,"Application Opened $newCount times",Toast.LENGTH_LONG).show()
        } else {
            editor.putInt("counterValue", 1)
            Toast.makeText(applicationContext,"Application Opened the first time",Toast.LENGTH_LONG).show()
        }
        editor.commit()
    }
